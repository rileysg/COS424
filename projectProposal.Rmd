...
```{r kdeGMM}
gmmKde <- function( Y, X, groupOrder){
  #remove any non-observed states from the factor datatype
  states <- unique( as.factor( Y))
  
  theta <- lapply( states, function( myState){
    #compute bandwith estimates for KDE
    bandwidths <- apply( X= X, MARGIN= 2, FUN= dpik)
    
    #store previous observations as parameters of the model
    list( observations= X[ Y == myState, ],
...
gmmKde.predict <- function( X,groupOrder){
  #TODO: use veterbi algorithm to make predictions from kernel density estimates
}
...
X <- X[ non.nan, ]
Y <- worm1.wide$behavior[ non.nan]
groupOrder <- worm1.wide$index[ non.nan]
```

```{r crossValidation}

foldCV <- function( training, test)
#partition the training data into training and hold out sets
